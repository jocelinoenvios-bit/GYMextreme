 ...
 * não autorizado, credencial corrompida). Sempre registra o evento e
 * monta a resposta no formato do Control iD, exatamente como o caminho
 * normal.
 */
async function negarDireto(db, { evento, dispositivo, motivo, inicio }) {
  ...
  return {
    httpStatus: 200,
    corpo: construirRespostaIdentificacao({
      resultado: RESULTADO.DENY,
      userIdDispositivo: evento.userIdDispositivo,
      userName: evento.userName,
      portalId: evento.portalId,
...
/**
 * Processa um evento `new_user_identified` do iDFace Pro de ponta a
 * ponta: resolve o dispositivo (`DeviceAuth`), interpreta o payload
 * (`ControlIdAccessProvider`), localiza o aluno, decide ALLOW/DENY
 * (`AccessAuthorizationService`) e registra o evento
 ...
 * esperando).
 *
 * Sempre responde HTTP 200 — mesmo em negativa (`event: 6` no corpo). A
 * segurança de "nunca abrir sem autorização" está inteiramente no
 * `event` do corpo da resposta, nunca no status HTTP: como o
 ...
 * aberto) em qualquer cenário.
 *
 * @param {FirebaseFirestore.Firestore} db
 * @param {{ payload: Record<string, unknown>, deviceToken: string|null|undefined }} entrada
 * @returns {Promise<{ httpStatus: number, corpo: ReturnType<typeof construirRespostaIdentificacao> }>}
 */
async function processarEventoIdentificacao(db, { payload, deviceToken }) {
  const inicio = Date.now();
  const evento = interpretarEventoIdentificacao(payload);
  ...
  try {
    ...
    if (evento.userIdDispositivo) {
      ...
      // teve esse usuario sincronizado (ver `DeviceSyncService`, ainda
      // nao implementado).
    }
    ...
    // usado pelo health check (seção 18) — independente do resultado da
    // decisao.
    await db
      .collection('dispositivosAcesso')
      .doc(dispositivo.id)
    ...
    if (motivoForcado) {
      return negarDireto(db, { evento, dispositivo, motivo: motivoForcado, inicio });
    }
    ...
    console.error('Erro ao processar evento new_user_identified:', err);
    try {
      await registrarEvento(
        db,
        construirRegistroEvento({
...
module.exports = { processarEventoIdentificacao };
